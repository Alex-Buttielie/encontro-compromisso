...
const tasks = [
  // ESPAÇO FÍSICO PARA O CANTEIRO DE OBRAS
  ...
  // ESPAÇO FÍSICO PARA MOMENTOS EXTRAS
  ...
  // IMPRESSOS, MATERIAIS GRÁFICOS E LEMBRANCINHAS
...
function seed() {
  ...
  // Seed Lembrancinhas (baseado nas responsabilidades das equipes)
  const lembrancinhas = [
  ...
  console.log(`Seeded: ${tasks.length} tasks, ${teams.length} teams, ${schedule.length} schedule items, ${escolinhas.length} escolinhas, ${alicerces.length} alicerces/alvenarias, ${lembrancinhas.length} lembrancinhas, ${financeItems.length} finance items, ${fornecedores.length} fornecedores, ${avisos.length} avisos`);
}

module.exports = { seed, tasks, teams, schedule };
