...
function reportHeader(doc, title, subtitle) {
  // Top bar
  doc.fillColor(COLORS.primary).rect(0, 0, doc.page.width, 75).fill();
  // Accent line
  doc.fillColor(COLORS.green).rect(0, 75, doc.page.width, 4).fill();
  // Brand
  doc.fillColor('#fff').fontSize(20).font('Helvetica-Bold').text('Meu Coordenador', 50, 18);
  doc.fontSize(9).font('Helvetica').fillColor('rgba(255,255,255,0.8)').text('JUMIRE - Projeto Compromisso Trin', 50, 45);
  // Date on right
  const dateStr = new Date().toLocaleDateString('pt-BR');
  doc.fontSize(8).fillColor('rgba(255,255,255,0.6)').text(dateStr, doc.page.width - 150, 20, { width: 100, align: 'right' });
  // Title
  doc.fillColor(COLORS.secondary).fontSize(16).font('Helvetica-Bold').text(title, 50, 95);
  if (subtitle) {
    doc.fontSize(9).font('Helvetica').fillColor(COLORS.gray).text(subtitle, 50, 118);
  }
}
...
function sectionTitle(doc, title, y, color) {
  ...
  doc.fillColor(c).rect(50, y, 4, 14).fill();
  doc.fillColor(c).fontSize(12).font('Helvetica-Bold').text(title, 60, y);
  return y + 22;
}

function summaryCard(doc, label, value, x, y, w, h, color) {
  doc.fillColor(color || COLORS.secondary).roundedRect(x, y, w, h, 5).fill();
  doc.fillColor('#fff').fontSize(h > 45 ? 20 : 14).font('Helvetica-Bold').text(value, x, y + 8, { width: w, align: 'center' });
  doc.fontSize(8).font('Helvetica').text(label, x, y + h - 14, { width: w, align: 'center' });
}

function progressBar(doc, pct, x, y, w) {
  doc.fillColor(COLORS.light).roundedRect(x, y, w, 10, 3).fill();
  const fillW = Math.max(2, (w * pct) / 100);
  const color = pct === 100 ? COLORS.green : pct >= 50 ? COLORS.orange : COLORS.red;
  doc.fillColor(color).roundedRect(x, y, fillW, 10, 3).fill();
}
...
function tableHeader(doc, columns, y) {
  doc.fillColor(COLORS.secondary).rect(50, y, CONTENT_WIDTH, 16, 2).fill();
  doc.fillColor('#fff').fontSize(8).font('Helvetica-Bold');
  for (const col of columns) {
    doc.text(col.label, col.x, y + 4, { width: col.w || 80, align: col.align || 'left' });
  }
  return y + 18;
}
...
function statusBadge(doc, status, x, y) {
  ...
  doc.fillColor(color).roundedRect(x, y, 80, 14, 3).fill();
  doc.fillColor('#fff').fontSize(7).font('Helvetica-Bold').text(label, x, y + 3, { width: 80, align: 'center' });
}
...
function generateFullReport() {
  ...
  cards.forEach((c, i) => {
    summaryCard(doc, c.label, c.value, 50 + i * (cardW + 10), y, cardW, 50, c.color);
  });

  y += 65;
  const pct = stats.total > 0 ? Math.round((stats.done / stats.total) * 100) : 0;
  doc.fillColor(COLORS.dark).fontSize(11).font('Helvetica-Bold').text(`Progresso Geral: ${pct}%`, 50, y);
  y += 16;
  ...
  for (const cat of categories) {
    ...
    for (const t of items) {
      ...
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(`[${t.item_number}]`, 68, y, { continued: true });
      doc.font('Helvetica').text(` ${t.title}`, { width: 370 });
      doc.fontSize(7).font('Helvetica').fillColor(COLORS.gray).text(`Equipe: ${t.responsible_team || 'N/A'} | Prazo: ${t.deadline || 'N/A'} | Prioridade: ${priorityLabel(t.priority)}`, 68, y + 12, { width: 370 });
  ...
  for (const day of days) {
    ...
    for (const s of items) {
      if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      zebraRow(doc, y, 20);
      doc.fillColor(COLORS.primary).fontSize(9).font('Helvetica-Bold').text(s.time || '-', 55, y + 3, { width: 55 });
      doc.fillColor(COLORS.dark).font('Helvetica').text(s.activity || '', 115, y + 3, { width: 270 });
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(s.responsible_team || '', 400, y + 3, { width: 80 });
      statusBadge(doc, s.status, 480, y + 1);
      y += 20;
    }
    y += 10;
  }
  ...
  for (const team of teams) {
    ...
    if (members.length > 0) {
      doc.fontSize(8).fillColor(COLORS.gray).font('Helvetica-Bold').text(`Membros (${members.length}):`, 50, y);
      y += 12;
      const colW = 250;
      members.forEach((m, i) => {
        const col = i % 2;
        ...
        doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`- ${m.name}${m.role ? ' (' + m.role + ')' : ''}${m.phone ? ' ' + m.phone : ''}`, mx, my, { width: colW - 5 });
      });
      y += Math.ceil(members.length / 2) * 12 + 8;
    }
    y += 8;
  }
...
function generateCategoryReport(category) {
  ...
  // Summary cards
  const cardW = 120;
  summaryCard(doc, 'Total', total, 50, y, cardW, 45, COLORS.secondary);
  summaryCard(doc, 'Concluidas', done, 60 + cardW, y, cardW, 45, COLORS.green);
  summaryCard(doc, 'Em Andamento', inProgress, 70 + cardW * 2, y, cardW, 45, COLORS.orange);
  summaryCard(doc, 'Pendentes', pending, 80 + cardW * 3, y, cardW, 45, COLORS.red);
  y += 55;

  doc.fillColor(COLORS.dark).fontSize(11).font('Helvetica-Bold').text(`Progresso: ${pct}%`, 50, y);
  y += 14;
  progressBar(doc, pct, 50, y, CONTENT_WIDTH);
  y += 25;
...
function generateTeamReport() {
  ...
  summaryCard(doc, 'Equipes', teams.length, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Tarefas', totalTasks, 180, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Concluidas', doneTasks, 310, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Progresso', overallPct + '%', 440, y, 120, 45, overallPct >= 50 ? COLORS.green : COLORS.orange);
  y += 60;
  ...
  for (const team of teams) {
    ...
    if (members.length > 0) {
      ...
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica-Oblique').text('Nenhum membro cadastrado', 55, y);
      y += 14;
    }
    y += 10;
  }
...
function generateTeamScheduleReport() {
  ...
  for (const team of teams) {
    ...
    const members = db.getAll('team_members').filter(m => m.team_id === team.id);
    if (members.length > 0) {
      doc.fontSize(8).fillColor(COLORS.gray).font('Helvetica-Bold').text('Membros: ', 50, y, { continued: true });
    ...
    if (teamSchedule.length > 0) {
      y += 4;
      doc.fillColor(COLORS.secondary).fontSize(10).font('Helvetica-Bold').text('Cronograma do Encontro', 50, y);
      y += 16;
      ...
      for (const s of teamSchedule) {
        ...
        if (s.day !== currentDay) {
          currentDay = s.day;
          doc.fillColor(COLORS.accent).fontSize(9).font('Helvetica-Bold').text(`  ${currentDay}`, 50, y);
          y += 12;
        }
        zebraRow(doc, y, 22);
        doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(`    ${s.time || '-'}`, 50, y + 3, { width: 60, continued: true });
        doc.font('Helvetica').fillColor(COLORS.dark).text(`  ${s.activity}`, { width: 400 });
        doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica').text(`    Local: ${s.location || '-'}`, 50, y + 13, { width: 400 });
        ...
        doc.fillColor(sColors[s.status] || COLORS.gray).roundedRect(470, y + 2, 80, 12, 3).fill();
        doc.fillColor('#fff').fontSize(7).font('Helvetica-Bold').text(sLabels[s.status] || s.status, 470, y + 4, { width: 80, align: 'center' });
        y += 22;
      }
    } else {
      y += 4;
      doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica-Oblique').text('  Sem atividades no cronograma.', 50, y);
      y += 14;
    }
...
function generateScheduleReport() {
  ...
  for (const day of days) {
    ...
    const items = db.getAll('schedule').filter(s => s.day === day).sort((a, b) => (a.time || '').localeCompare(b.time || ''));
    if (items.length === 0) {
      doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica-Oblique').text('Nenhuma atividade cadastrada.', 55, y);
      y += 16;
    }
    for (const s of items) {
      if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      ...
      doc.fillColor(COLORS.primary).fontSize(9).font('Helvetica-Bold').text(s.time || '-', 55, y + 3, { width: 55 });
      doc.fillColor(COLORS.dark).font('Helvetica').text(s.activity || '', 115, y + 3, { width: 270 });
      doc.fillColor(COLORS.gray).fontSize(8).text(s.location || '', 400, y + 3, { width: 100 });
      doc.fillColor(COLORS.secondary).fontSize(8).font('Helvetica-Bold').text(s.responsible_team || '', 400, y + 14, { width: 100 });
      statusBadge(doc, s.status, 480, y + 2);
      y += 24;
    }
    y += 12;
  }
...
function generateParticipantsReport() {
  ...
  // Summary cards
  summaryCard(doc, 'Total', total, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Pagos', paid, 180, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Pendentes', total - paid, 310, y, 120, 45, COLORS.orange);
  summaryCard(doc, 'Com Restricoes', restricted.length, 440, y, 120, 45, COLORS.red);
  y += 60;
  ...
  for (let i = 0; i < participants.length; i++) {
    ...
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 16);
    doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(String(p.id), 54, y + 3, { width: 20 });
    doc.text(p.name || '-', 78, y + 3, { width: 120 });
    doc.text(p.cracha_name || '-', 200, y + 3, { width: 70 });
    doc.text(String(p.age || '-'), 272, y + 3, { width: 30 });
    doc.text(p.group || '-', 306, y + 3, { width: 60 });
    doc.text(p.room || '-', 368, y + 3, { width: 50 });
    doc.text(p.shirt_size || '-', 420, y + 3, { width: 40 });
    doc.fillColor(p.paid ? COLORS.green : COLORS.red).font('Helvetica-Bold').text(p.paid ? 'Sim' : 'Nao', 470, y + 3, { width: 30, align: 'center' });
    doc.fillColor(p.food_restriction ? COLORS.red : COLORS.gray).font('Helvetica').text(p.food_restriction ? '! ' + p.food_restriction : '', 500, y + 3, { width: 60 });
    y += 16;
  }
  ...
  if (restricted.length === 0) {
    ...
    for (const p of restricted) {
      if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      zebraRow(doc, y, 36);
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(p.name, 55, y);
      y += 12;
      if (p.food_restriction) { doc.fillColor(COLORS.red).font('Helvetica').text(`  Restricao alimentar: ${p.food_restriction}`, 55, y, { width: 480 }); y += 11; }
      if (p.medication) { doc.fillColor(COLORS.orange).font('Helvetica').text(`  Medicacao: ${p.medication}`, 55, y, { width: 480 }); y += 11; }
      if (p.special_needs) { doc.fillColor(COLORS.secondary).font('Helvetica').text(`  Necessidades especiais: ${p.special_needs}`, 55, y, { width: 480 }); y += 11; }
      y += 6;
    }
  }
...
function generateFinanceReport() {
  ...
  let y = 145;
  // Summary cards
  summaryCard(doc, 'Receitas', fmtMoney(totalIncome), 50, y, 120, 50, COLORS.green);
  summaryCard(doc, 'Despesas', fmtMoney(totalExpenses), 180, y, 120, 50, COLORS.red);
  summaryCard(doc, 'Saldo', fmtMoney(balance), 310, y, 120, 50, balance >= 0 ? COLORS.green : COLORS.red);
  summaryCard(doc, 'A Receber', fmtMoney(pendingIncome), 440, y, 120, 50, COLORS.orange);
  y += 65;

  // Pending expenses
  if (pendingExpenses > 0) {
    doc.fillColor(COLORS.red).fontSize(9).font('Helvetica-Bold').text(`A Pagar: ${fmtMoney(pendingExpenses)}`, 50, y);
    y += 16;
  }
  ...
  for (const [cat, vals] of Object.entries(cats)) {
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 14);
    doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(cat, 54, y + 2, { width: 200 });
    doc.fillColor(COLORS.green).text(fmtMoney(vals.receita || 0), 280, y + 2, { width: 100, align: 'right' });
    doc.fillColor(COLORS.red).text(fmtMoney(vals.despesa || 0), 400, y + 2, { width: 100, align: 'right' });
    const s = (vals.receita || 0) - (vals.despesa || 0);
    doc.fillColor(s >= 0 ? COLORS.green : COLORS.red).font('Helvetica-Bold').text(fmtMoney(s), 500, y + 2, { width: 50, align: 'right' });
    y += 14;
  }
  ...
  if (sortedMonths.length > 0) {
    ...
    for (const m of sortedMonths) {
      ...
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(monthLabel, 54, y + 2, { width: 150 });
      doc.fillColor(COLORS.green).text(fmtMoney(months[m].receita || 0), 280, y + 2, { width: 100, align: 'right' });
      doc.fillColor(COLORS.red).text(fmtMoney(months[m].despesa || 0), 400, y + 2, { width: 100, align: 'right' });
      const ms = (months[m].receita || 0) - (months[m].despesa || 0);
      doc.fillColor(ms >= 0 ? COLORS.green : COLORS.red).font('Helvetica-Bold').text(fmtMoney(ms), 500, y + 2, { width: 50, align: 'right' });
      y += 14;
    }
  }
  ...
  for (const i of items) {
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 26);
    doc.fillColor(i.type === 'receita' ? COLORS.green : COLORS.red).circle(55, y + 5, 3).fill();
    doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(i.type === 'receita' ? '+' : '-', 66, y, { continued: true });
    doc.font('Helvetica').text(` ${fmtMoney(i.amount)} - ${i.description || ''}`, { width: 350 });
    doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica').text(`${i.category || ''} | ${i.date ? fmtDate(i.date) : ''} | ${i.paid ? 'Pago' : 'Pendente'} | Resp: ${i.responsible || '-'}`, 66, y + 12, { width: 440 });
    y += 26;
  }
...
function generateAlicercesReport() {
  ...
  // Summary cards
  summaryCard(doc, 'Alicerces', alicerceItems.length, 50, y, 120, 45, COLORS.primary);
  summaryCard(doc, 'Concluidos', alicerceDone, 180, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Alvenarias', alvenariaItems.length, 310, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Concluidas', alvenariaDone, 440, y, 120, 45, COLORS.green);
  y += 60;
  ...
  for (const a of alicerceItems) {
    if (y > PAGE_BOTTOM - 40) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 40);
    doc.fillColor(COLORS.primary).fontSize(11).font('Helvetica-Bold').text(a.title, 55, y);
    y += 14;
    if (a.content) { doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(a.content, 55, y, { width: 480 }); y += Math.ceil(doc.widthOfString(a.content, { width: 480 }) / 480) * 12 + 4; }
    doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(`Construtor: ${a.builder || 'Nao atribuido'} | Dia: ${a.day || '-'} | Horario: ${a.time || '-'}`, 55, y, { width: 400 });
    y += 12;
    ...
    doc.fillColor(stColors[a.status] || COLORS.gray).roundedRect(55, y, 80, 12, 3).fill();
    doc.fillColor('#fff').fontSize(7).font('Helvetica-Bold').text(stLabels[a.status] || a.status || 'pendente', 55, y + 2, { width: 80, align: 'center' });
    y += 20;
  }
  ...
  for (const a of alvenariaItems) {
    if (y > PAGE_BOTTOM - 40) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 40);
    doc.fillColor(COLORS.secondary).fontSize(11).font('Helvetica-Bold').text(a.title, 55, y);
    y += 14;
    if (a.content) { doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(a.content, 55, y, { width: 480 }); y += Math.ceil(doc.widthOfString(a.content, { width: 480 }) / 480) * 12 + 4; }
    doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(`Construtor: ${a.builder || 'Nao atribuido'} | Dia: ${a.day || '-'} | Horario: ${a.time || '-'}`, 55, y, { width: 400 });
    y += 12;
...
function generateLembrancinhasReport() {
  ...
  // Summary cards
  summaryCard(doc, 'Total', items.length, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Prontas', done, 180, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Em Andamento', inProgress, 310, y, 120, 45, COLORS.orange);
  summaryCard(doc, 'Nao Iniciadas', notStarted, 440, y, 120, 45, COLORS.gray);
  y += 60;
  ...
  for (const l of items) {
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 18);
    doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(l.team || '-', 54, y + 3, { width: 120 });
    doc.text(l.item_name || '-', 180, y + 3, { width: 150 });
    doc.text(String(l.quantity_needed || 0), 340, y + 3, { width: 60, align: 'center' });
    doc.text(String(l.quantity_done || 0), 410, y + 3, { width: 50, align: 'center' });
    ...
    doc.fillColor(stColors[l.status] || COLORS.gray).roundedRect(470, y + 1, 70, 12, 3).fill();
    doc.fillColor('#fff').fontSize(7).font('Helvetica-Bold').text(stLabels[l.status] || l.status || '-', 470, y + 3, { width: 70, align: 'center' });
    y += 18;
  }
...
function generateFornecedoresReport() {
  ...
  // Summary cards
  summaryCard(doc, 'Fornecedores', fornecedores.length, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Contratados', contratados, 180, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Pendentes', pendentes, 310, y, 120, 45, COLORS.orange);
  summaryCard(doc, 'Pais de MPs', paisMP.length, 440, y, 120, 45, COLORS.primary);
  y += 60;
  ...
  for (const cat of cats) {
    ...
    doc.moveTo(55, y + 13).lineTo(560, y + 13).strokeColor(COLORS.green).lineWidth(1).stroke();
    y += 18;
    const catItems = fornecedores.filter(f => f.category === cat);
    for (const f of catItems) {
      if (y > PAGE_BOTTOM - 40) { doc.addPage(); y = 50; }
      zebraRow(doc, y, 50);
      doc.fillColor(COLORS.dark).fontSize(10).font('Helvetica-Bold').text(f.name || 'Sem nome', 55, y);
      y += 13;
      ...
      if (contacts.length) { doc.text(`  ${contacts.join(' | ')}`, 65, y, { width: 440 }); y += 11; }
      ...
      doc.fillColor(stColors[f.status] || COLORS.gray).roundedRect(470, y - 11, 80, 12, 3).fill();
      doc.fillColor('#fff').fontSize(7).font('Helvetica-Bold').text(stLabels[f.status] || f.status, 470, y - 9, { width: 80, align: 'center' });
      if (f.notes) { doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica-Oblique').text(`Obs: ${f.notes}`, 65, y, { width: 440 }); y += 11; }
      y += 8;
    }
    y += 8;
  }
  ...
  if (paisMP.length > 0) {
    ...
    for (const f of paisMP) {
      ...
      if (details.length) { doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica').text(details.join(' | '), 65, y, { width: 440 }); y += 11; }
      const stColors = { contratado: COLORS.green, pendente: COLORS.orange, contatado: COLORS.secondary, cancelado: COLORS.red };
...
function generateAvisosReport() {
  ...
  summaryCard(doc, 'Total', items.length, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Fixados', pinned, 180, y, 120, 45, COLORS.primary);
  summaryCard(doc, 'Prioridade Alta', alta, 310, y, 120, 45, COLORS.red);
  summaryCard(doc, 'Outros', items.length - alta, 440, y, 120, 45, COLORS.gray);
  y += 60;

  if (items.length === 0) {
    doc.fillColor(COLORS.gray).fontSize(12).font('Helvetica').text('Nenhum aviso publicado.', 55, y);
  } else {
    for (const a of items) {
      if (y > PAGE_BOTTOM - 60) { doc.addPage(); y = 50; }
      const pColors = { alta: COLORS.red, media: COLORS.orange, baixa: COLORS.secondary };
      zebraRow(doc, y, 70);
      doc.fillColor(pColors[a.priority] || COLORS.gray).rect(50, y, 4, 60).fill();
      doc.fillColor(COLORS.dark).fontSize(12).font('Helvetica-Bold').text(`${a.pinned ? '- ' : ''}${a.title}`, 60, y);
      y += 16;
...
function generateKitReport() {
  ...
  let y = 145;
  const participants = db.getAll('participants');
  summaryCard(doc, 'Itens do Kit', 10, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Inscritos', participants.length, 180, y, 120, 45, COLORS.primary);
  summaryCard(doc, 'Pagos', participants.filter(p => p.paid).length, 310, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Pendentes', participants.filter(p => !p.paid).length, 440, y, 120, 45, COLORS.orange);
  y += 60;

  doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica-Oblique').text('Conferir os Kits ate 30 dias antes do Encontro. Cada materia-prima deve receber todos os itens abaixo.', 50, y, { width: CONTENT_WIDTH });
  y += 18;
  ...
  for (const k of kitItems) {
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 18);
    doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(k.item, 54, y + 3, { width: 220 });
    doc.fillColor(COLORS.gray).text(k.momento, 280, y + 3, { width: 150 });
    doc.text(k.anexo, 440, y + 3, { width: 100 });
    doc.strokeColor(COLORS.gray).lineWidth(1).rect(530, y + 1, 14, 14).stroke();
    y += 18;
  }
  ...
  if (participants.length === 0) {
    doc.fillColor(COLORS.gray).fontSize(10).font('Helvetica').text('Nenhuma materia-prima inscrita ainda.', 55, y);
  } else {
    y = tableHeader(doc, [
      { label: '#', x: 54, w: 20 },
    ...
    for (const p of participants) {
      ...
      zebraRow(doc, y, 16);
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(String(p.id), 54, y + 3, { width: 20 });
      doc.text(p.name || '-', 78, y + 3, { width: 200 });
      doc.strokeColor(COLORS.gray).rect(310, y + 1, 12, 12).stroke();
      doc.strokeColor(COLORS.gray).rect(410, y + 1, 12, 12).stroke();
      doc.strokeColor(COLORS.gray).rect(500, y + 1, 50, 12).stroke();
      y += 16;
    }
  }
...
function generateCoordinatorGuideReport() {
  ...
  if (mo.length > 0) {
    for (const m of mo) {
      ...
      zebraRow(doc, y, 14);
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(`${m.name}`, 55, y + 2, { continued: true });
      doc.font('Helvetica').fillColor(COLORS.gray).text(`  - ${t ? t.name : ''}  ${m.phone ? '| ' + m.phone : ''}  ${m.role ? '| ' + m.role : ''}`);
      y += 13;
    }
  } else {
    doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica').text('Nenhum Mestre de Obras ou Supervisor cadastrado.', 55, y);
    y += 14;
  }
  y += 12;
  ...
  for (const team of teams) {
    if (y > PAGE_BOTTOM - 60) { doc.addPage(); y = 50; }
    doc.fillColor(COLORS.secondary).fontSize(10).font('Helvetica-Bold').text(team.name, 55, y);
    y += 14;
    if (team.description) {
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(team.description, 55, y, { width: CONTENT_WIDTH });
      y += 12;
    }
    ...
    if (tm.length > 0) {
      for (const m of tm) {
        if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
        doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`  - ${m.name}${m.role ? ' - ' + m.role : ''}${m.phone ? '  | ' + m.phone : ''}`, 55, y, { width: CONTENT_WIDTH });
        y += 11;
      }
    } else {
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica-Oblique').text('  Sem membros cadastrados', 55, y);
      y += 11;
    }
    y += 8;
  }
  ...
  for (const a of alicerceItems) {
    if (y > PAGE_BOTTOM - 40) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 40);
    doc.fillColor(COLORS.primary).fontSize(10).font('Helvetica-Bold').text(a.title, 55, y);
    y += 14;
    doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`Construtor: ${a.builder || 'NAO ATRIBUIDO'}  |  Dia: ${a.day || '-'}  |  Horario: ${a.time || '-'}`, 55, y, { width: CONTENT_WIDTH });
    y += 14;
    if (a.content) {
      const lines = Math.ceil(doc.widthOfString(a.content, { width: 480 }) / 480) * 10;
      if (y + lines > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(a.content, 65, y, { width: 480 });
      y += lines + 6;
    }
    y += 6;
  }
  ...
  for (const a of alvenariaItems) {
    if (y > PAGE_BOTTOM - 40) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 40);
    doc.fillColor(COLORS.secondary).fontSize(10).font('Helvetica-Bold').text(a.title, 55, y);
    y += 14;
    doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`Construtor: ${a.builder || 'NAO ATRIBUIDO'}  |  Dia: ${a.day || '-'}  |  Horario: ${a.time || '-'}`, 55, y, { width: CONTENT_WIDTH });
    y += 14;
    if (a.content) {
  ...
  summaryCard(doc, 'Total', totalMP, 50, y, 120, 45, COLORS.secondary);
  summaryCard(doc, 'Pagos', paidMP, 180, y, 120, 45, COLORS.green);
  summaryCard(doc, 'Pendentes', totalMP - paidMP, 310, y, 120, 45, COLORS.orange);
  summaryCard(doc, 'Com Restricoes', restrictedMP.length, 440, y, 120, 45, COLORS.red);
  y += 60;
  ...
  for (const p of participants) {
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    zebraRow(doc, y, 13);
    doc.fillColor(COLORS.dark).fontSize(7).font('Helvetica').text(String(p.id), 54, y + 2, { width: 16 });
    doc.text(p.name || '-', 72, y + 2, { width: 110 });
    doc.text(p.group || '-', 184, y + 2, { width: 50 });
    doc.text(p.room || '-', 236, y + 2, { width: 45 });
    doc.text(p.shirt_size || '-', 283, y + 2, { width: 35 });
    doc.fillColor(p.paid ? COLORS.green : COLORS.red).font('Helvetica-Bold').text(p.paid ? 'S' : 'N', 320, y + 2, { width: 12, align: 'center' });
    doc.fillColor(p.food_restriction ? COLORS.red : COLORS.gray).font('Helvetica').text(p.food_restriction || '-', 335, y + 2, { width: 100 });
    doc.fillColor(p.medication ? COLORS.orange : COLORS.gray).text(p.medication || '-', 437, y + 2, { width: 80 });
    doc.fillColor(p.special_needs ? COLORS.secondary : COLORS.gray).text(p.special_needs || '-', 520, y + 2, { width: 40 });
    y += 13;
  }
  ...
  if (restricted.length === 0) {
    doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica').text('Nenhuma restricao cadastrada.', 55, y);
    y += 12;
  } else {
    for (const p of restricted) {
      if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      zebraRow(doc, y, 36);
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica-Bold').text(p.name, 55, y);
      y += 11;
      if (p.food_restriction) { doc.fillColor(COLORS.red).font('Helvetica').text(`  ! Restricao alimentar: ${p.food_restriction}`, 55, y); y += 10; }
      if (p.medication) { doc.fillColor(COLORS.orange).text(`  Medicacao: ${p.medication}`, 55, y); y += 10; }
      if (p.special_needs) { doc.fillColor(COLORS.secondary).text(`  Necessidades especiais: ${p.special_needs}`, 55, y); y += 10; }
      y += 6;
    }
  }
  ...
  if (padrinhos.length === 0) {
    doc.fillColor(COLORS.gray).fontSize(10).font('Helvetica').text('Nenhum padrinho atribuido ainda.', 55, y);
  } else {
    y = tableHeader(doc, [
    ...
    for (const pad of padrinhos) {
      ...
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(mp ? mp.name : '-', 54, y + 3, { width: 120 });
      doc.text(pad.name || '-', 176, y + 3, { width: 100 });
      const steps = ['step1_contact', 'step2_invite', 'step3_confirm', 'step4_meeting', 'step5_follow'];
      steps.forEach((s, i) => {
        const x = 278 + i * 45;
        if (pad[s]) { doc.fillColor(COLORS.green).circle(x + 22, y + 6, 3).fill(); }
        else { doc.strokeColor(COLORS.gray).lineWidth(0.5).circle(x + 22, y + 6, 3).stroke(); }
      });
      const stColors = { completo: COLORS.green, em_andamento: COLORS.orange, atribuido: COLORS.gray };
      doc.fillColor(stColors[pad.status] || COLORS.gray).roundedRect(490, y, 65, 10, 2).fill();
      doc.fillColor('#fff').fontSize(6).font('Helvetica-Bold').text(pad.status || '-', 490, y + 2, { width: 65, align: 'center' });
      y += 14;
    }
  }
  ...
  if (avisos.length === 0) {
    doc.fillColor(COLORS.gray).fontSize(10).font('Helvetica').text('Nenhum aviso publicado.', 55, y);
  } else {
    for (const a of avisos) {
      if (y > PAGE_BOTTOM - 60) { doc.addPage(); y = 50; }
      const pColors = { alta: COLORS.red, media: COLORS.orange, baixa: COLORS.secondary };
      zebraRow(doc, y, 60);
      doc.fillColor(pColors[a.priority] || COLORS.gray).rect(50, y, 4, 50).fill();
      doc.fillColor(COLORS.dark).fontSize(12).font('Helvetica-Bold').text(`${a.pinned ? '- ' : ''}${a.title}`, 60, y);
      y += 16;
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(a.content || '', 60, y, { width: 480 });
      y += Math.ceil(doc.widthOfString(a.content || '', { width: 480 }) / 480) * 12 + 6;
      doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica').text(`Publico: ${targetLabels[a.target] || a.target} | Prioridade: ${priorityLabels[a.priority] || a.priority} | ${a.created_at ? new Date(a.created_at).toLocaleDateString('pt-BR') : ''}`, 60, y, { width: 480 });
      y += 16;
      doc.strokeColor(COLORS.light).lineWidth(1).moveTo(50, y).lineTo(560, y).stroke();
      y += 12;
    }
  ...
  summaryCard(doc, 'Pendentes', pending.length, 50, y, 120, 45, COLORS.red);
  summaryCard(doc, 'Em Andamento', inProgress.length, 180, y, 120, 45, COLORS.orange);
  summaryCard(doc, 'Nao Iniciadas', notStarted.length, 310, y, 120, 45, COLORS.gray);
  summaryCard(doc, 'Concluidas', tasks.length - pending.length, 440, y, 120, 45, COLORS.green);
  y += 60;
  ...
  if (inProgress.length > 0) {
    ...
    for (const t of inProgress) {
      if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      zebraRow(doc, y, 22);
      doc.fillColor(COLORS.orange).circle(55, y + 4, 3).fill();
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica-Bold').text(`[${t.item_number}]`, 65, y, { continued: true, width: 30 });
      doc.font('Helvetica').text(` ${t.title}`, { width: 380 });
      doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica').text(`Equipe: ${t.responsible_team || '-'} | Prazo: ${t.deadline || '-'}`, 65, y + 11, { width: 380 });
      y += 22;
    }
    y += 8;
  }
  ...
  if (notStarted.length > 0) {
    ...
    for (const t of notStarted) {
      if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
      zebraRow(doc, y, 20);
      doc.strokeColor(COLORS.gray).lineWidth(0.5).circle(55, y + 4, 3).stroke();
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`[${t.item_number}] ${t.title}`, 65, y, { width: 400 });
      doc.fillColor(COLORS.gray).fontSize(7).text(`Equipe: ${t.responsible_team || '-'} | Prazo: ${t.deadline || '-'}`, 65, y + 11, { width: 400 });
      y += 20;
    }
  }

  if (pending.length === 0) {
    doc.fillColor(COLORS.green).fontSize(12).font('Helvetica-Bold').text('Todas as tarefas estao concluidas!', 55, y);
  }
  ...
  for (const cat of cats) {
    ...
    for (const f of catItems) {
      ...
      zebraRow(doc, y, 36);
      const stColors = { contratado: COLORS.green, pendente: COLORS.orange, contatado: COLORS.secondary, cancelado: COLORS.red };
      doc.fillColor(stColors[f.status] || COLORS.gray).circle(55, y + 4, 3).fill();
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(f.name || '-', 65, y, { width: 200 });
      y += 11;
      const contacts = [];
      if (f.contact_person) contacts.push(f.contact_person);
      if (f.phone) contacts.push(f.phone);
      if (f.whatsapp) contacts.push(`WhatsApp: ${f.whatsapp}`);
      if (contacts.length) { doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(`  ${contacts.join(' | ')}`, 65, y, { width: 450 }); y += 10; }
      if (f.service) { doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica').text(`  Servico: ${f.service}`, 65, y, { width: 450 }); y += 9; }
      y += 6;
    }
    y += 6;
  }
  ...
  doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica-Oblique').text('Use este espaco para anotar observacoes, imprevistos, ideias e lembretes durante os dias do Encontro.', 50, y, { width: CONTENT_WIDTH });
  y += 24;

  for (let i = 0; i < 20; i++) {
    if (y > PAGE_BOTTOM) { doc.addPage(); y = 50; }
    doc.strokeColor(COLORS.light).lineWidth(0.5).moveTo(50, y).lineTo(560, y).stroke();
    y += 24;
  }
...
function generatePreparationReport() {
  ...
  cards.forEach((c, i) => {
    ...
    doc.fillColor(c.color).roundedRect(x, y, cardW, 50, 5).fill();
    doc.fillColor('#fff').fontSize(22).font('Helvetica-Bold').text(c.value, x, y + 8, { width: cardW, align: 'center' });
    doc.fontSize(9).font('Helvetica').text(c.label, x, y + 35, { width: cardW, align: 'center' });
  });

  y += 70;
  doc.fillColor(COLORS.dark).fontSize(12).font('Helvetica-Bold').text(`Progresso Geral: ${pct}%`, 50, y);
  y += 20;
  doc.fillColor(COLORS.light).rect(50, y, 510, 12).fill();
  doc.fillColor(COLORS.green).rect(50, y, (510 * pct) / 100, 12).fill();
  y += 30;
  ...
  doc.fontSize(10).font('Helvetica-Bold').fillColor(COLORS.secondary).text('Indicadores Gerais', 50, y);
  y += 18;
  for (const [label, value, color] of rows) {
    if (y > 760) { doc.addPage(); y = 50; }
    doc.fillColor(COLORS.light).rect(50, y, 510, 18, 2).fill();
  ...
  if (encStart) {
    ...
    if (overdueParsed.length > 0) {
      ...
      doc.fillColor(COLORS.red).fontSize(12).font('Helvetica-Bold').text(`Tarefas Atrasadas: ${overdueParsed.length}`, 50, y);
      y += 20;
      for (const t of overdueParsed.slice(0, 20)) {
        if (y > 760) { doc.addPage(); y = 50; }
        doc.fillColor(COLORS.red).circle(55, y + 4, 3).fill();
        doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica-Bold').text(t.title, 65, y, { width: 340 });
        doc.fillColor(COLORS.gray).font('Helvetica').text(`${t.category}`, 65, y + 11, { width: 340 });
        doc.fillColor(COLORS.red).font('Helvetica-Bold').text(`${t.diff_days}d atrasado`, 420, y, { width: 130, align: 'right' });
        doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(`Equipe: ${t.responsible_team || 'N/A'}`, 420, y + 11, { width: 130, align: 'right' });
        y += 26;
      }
  ...
  for (const cat of categories) {
    ...
    doc.fillColor(catColor).rect(50, y, (510 * catPct) / 100, 6).fill();
    y += 12;

    for (const t of catItems) {
      if (y > 760) { doc.addPage(); y = 50; }
      const sColor = t.status === 'concluido' ? COLORS.green : t.status === 'em_andamento' ? COLORS.orange : COLORS.gray;
      doc.fillColor(sColor).circle(55, y + 3, 3).fill();
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`[${t.item_number}] ${t.title}`, 65, y, { width: 350 });
      doc.fillColor(COLORS.gray).fontSize(7).text(`${t.responsible_team || ''} | ${t.deadline || ''}`, 65, y + 9, { width: 350 });
      statusBadge(doc, t.status, 470, y);
      y += 22;
    }
    y += 8;
  }
  ...
  for (const team of allTeams) {
    ...
    doc.fillColor(COLORS.gray).fontSize(9).font('Helvetica').text(`${team.member_count} membro(s)`, 450, y, { width: 110, align: 'right' });
    y += 16;
    if (team.description) {
    ...
    if (members.length > 0) {
      ...
      members.forEach((m, i) => {
        ...
        const mx = 55 + col * colW;
        const my = y + row * 12;
        if (my > 760) { doc.addPage(); y = 50; }
        doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(`- ${m.name}${m.role ? ' (' + m.role + ')' : ''}${m.phone ? ' ' + m.phone : ''}`, mx, my, { width: colW - 5 });
      });
      y += Math.ceil(members.length / 2) * 12 + 10;
    } else {
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica-Oblique').text('Nenhum membro cadastrado', 55, y);
      y += 14;
    }
    ...
    if (teamTasks.length > 0) {
      ...
      doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica').text(`Tarefas: ${tDone}/${teamTasks.length} concluidas`, 55, y);
      y += 12;
    }
    y += 8;
  }
  ...
  finCards.forEach((c, i) => {
    const x = 50 + i * 175;
    doc.fillColor(c.color).roundedRect(x, y, 160, 45, 5).fill();
    doc.fillColor('#fff').fontSize(14).font('Helvetica-Bold').text(c.value, x, y + 8, { width: 160, align: 'center' });
    doc.fontSize(9).font('Helvetica').text(c.label, x, y + 30, { width: 160, align: 'center' });
  });
  y += 65;
  ...
  doc.fillColor(COLORS.secondary).fontSize(11).font('Helvetica-Bold').text('Resumo por Categoria', 50, y);
  y += 18;
  doc.fillColor(COLORS.secondary).fontSize(8).font('Helvetica-Bold').text('Categoria', 50, y, { width: 200 });
  doc.text('Receitas', 270, y, { width: 100, align: 'right' });
  doc.text('Despesas', 380, y, { width: 100, align: 'right' });
  doc.text('Saldo', 490, y, { width: 70, align: 'right' });
  y += 14;
  for (const [cat, vals] of Object.entries(finCats).sort((a, b) => b[1].receita - b[1].despesa - (a[1].receita - a[1].despesa))) {
    ...
    doc.fillColor(COLORS.light).rect(50, y - 2, 510, 16, 2).fill();
    doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(cat, 55, y + 1, { width: 200 });
    doc.fillColor(COLORS.green).text(`R$ ${vals.receita.toFixed(2)}`, 270, y + 1, { width: 100, align: 'right' });
    doc.fillColor(COLORS.red).text(`R$ ${vals.despesa.toFixed(2)}`, 380, y + 1, { width: 100, align: 'right' });
    doc.fillColor(saldo >= 0 ? COLORS.green : COLORS.red).font('Helvetica-Bold').text(`R$ ${saldo.toFixed(2)}`, 490, y + 1, { width: 70, align: 'right' });
    y += 16;
  }
  ...
  doc.text('Padrinho', 240, y, { width: 150 });
  ...
  for (const p of prepParticipants) {
    ...
    doc.fillColor(COLORS.gray).text(p.padrinho || '-', 240, y, { width: 150 });
    doc.text(p.group || '-', 400, y, { width: 60 });
    doc.fillColor(p.paid ? COLORS.green : COLORS.orange).font('Helvetica-Bold').text(p.paid ? 'Pago' : 'Pendente', 470, y, { width: 80, align: 'right' });
    y += 14;
  }
  ...
  doc.text('Telefone', 430, y, { width: 80 });
  doc.text('Status', 520, y, { width: 40 });
  y += 14;
  for (const f of allForn) {
    if (y > 760) { doc.addPage(); y = 50; }
    doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(f.name || '-', 50, y, { width: 150 });
    doc.fillColor(COLORS.gray).text(f.category || '-', 210, y, { width: 100 });
    doc.text(f.contact_person || '-', 320, y, { width: 100 });
    doc.text(f.phone || f.whatsapp || '-', 430, y, { width: 80 });
    const stColor = f.status === 'confirmado' ? COLORS.green : f.status === 'em_negociacao' ? COLORS.orange : COLORS.gray;
    doc.fillColor(stColor).font('Helvetica-Bold').text(f.status || '-', 520, y, { width: 40 });
    y += 14;
  }
  ...
  if (allEsc.length === 0) {
    ...
    doc.fillColor(COLORS.secondary).fontSize(8).font('Helvetica-Bold').text('Data', 50, y, { width: 70 });
    doc.text('Hora', 130, y, { width: 50 });
    doc.text('Nome', 190, y, { width: 180 });
    doc.text('Local', 380, y, { width: 100 });
    doc.text('Status', 490, y, { width: 70 });
    y += 14;
    for (const e of allEsc) {
      if (y > 760) { doc.addPage(); y = 50; }
      const dt = e.date ? new Date(e.date + 'T00:00:00').toLocaleDateString('pt-BR') : '-';
      doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(dt, 50, y, { width: 70 });
      doc.text(e.time || '-', 130, y, { width: 50 });
      doc.font('Helvetica-Bold').text(e.name || '-', 190, y, { width: 180 });
      doc.font('Helvetica').fillColor(COLORS.gray).text(e.location || '-', 380, y, { width: 100 });
      const stColor = e.status === 'concluido' ? COLORS.green : e.status === 'em_andamento' ? COLORS.orange : COLORS.gray;
      doc.fillColor(stColor).font('Helvetica-Bold').text(e.status || '-', 490, y, { width: 70 });
      y += 16;
      if (e.description) {
        doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica-Oblique').text(e.description, 190, y, { width: 370 });
        y += 10;
      }
      y += 4;
    }
  ...
  if (allAvisos.length === 0) {
    ...
    for (const a of allAvisos) {
      ...
      doc.fillColor(pColor).roundedRect(50, y, 510, 4, 2).fill();
      y += 8;
      if (a.pinned) {
        doc.fillColor(COLORS.primary).fontSize(8).font('Helvetica-Bold').text('FIXADO', 50, y);
        y += 12;
      }
      doc.fillColor(COLORS.dark).fontSize(10).font('Helvetica-Bold').text(a.title, 50, y, { width: 510 });
      y += 16;
      if (a.content) {
        doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(a.content, 50, y, { width: 510 });
        y += Math.ceil(doc.widthOfString(a.content, { width: 510 }) / 510) * 12 + 6;
      }
      doc.fillColor(COLORS.gray).fontSize(7).font('Helvetica').text(`Prioridade: ${a.priority || 'baixa'} | Autor: ${a.author || 'N/A'}`, 50, y);
      y += 20;
    }
  }
  ...
  if (encStart) {
    ...
    if (autoLembretes.length === 0) {
      doc.fillColor(COLORS.gray).fontSize(10).font('Helvetica-Oblique').text('Nenhum prazo pendente. Todas as tarefas com prazo estao concluidas.', 50, y);
    } else {
      doc.fillColor(COLORS.secondary).fontSize(8).font('Helvetica-Bold').text('Prazo', 50, y, { width: 70 });
      doc.text('Urgencia', 130, y, { width: 70 });
      doc.text('Tarefa', 210, y, { width: 200 });
      doc.text('Equipe', 420, y, { width: 80 });
      doc.text('Dias', 510, y, { width: 50, align: 'right' });
      y += 14;
      for (const l of autoLembretes) {
        if (y > 760) { doc.addPage(); y = 50; }
        const dt = new Date(l.due_date + 'T00:00:00').toLocaleDateString('pt-BR');
        doc.fillColor(COLORS.dark).fontSize(8).font('Helvetica').text(dt, 50, y, { width: 70 });
        doc.fillColor(l.uColor).font('Helvetica-Bold').text(l.urgency, 130, y, { width: 70 });
        doc.fillColor(COLORS.dark).font('Helvetica').text(l.title, 210, y, { width: 200 });
        doc.fillColor(COLORS.gray).text(l.team || '-', 420, y, { width: 80 });
        doc.fillColor(l.diff_days < 0 ? COLORS.red : COLORS.dark).font('Helvetica-Bold').text(String(l.diff_days), 510, y, { width: 50, align: 'right' });
        y += 14;
      }
    }
  } else {
    doc.fillColor(COLORS.gray).fontSize(10).font('Helvetica-Oblique').text('Defina a data do Encontro para gerar prazos automaticos.', 50, y);
  }
  ...
  const checklistItems = [
    { area: 'Espaco Fisico', items: ['Canteiro de Obras reservado e confirmado', 'Aluguel pago (ou combinado)', 'Capela montada e equipada', 'Sala de reuniao para coordenação'] },
  ...
  for (const section of checklistItems) {
    if (y > 700) { doc.addPage(); y = 50; }
    doc.fillColor(COLORS.primary).fontSize(11).font('Helvetica-Bold').text(section.area, 50, y);
    y += 16;
    for (const item of section.items) {
      if (y > 760) { doc.addPage(); y = 50; }
      doc.strokeColor(COLORS.gray).lineWidth(0.5).roundedRect(55, y, 8, 8, 1).stroke();
      doc.fillColor(COLORS.dark).fontSize(9).font('Helvetica').text(item, 70, y, { width: 480 });
      y += 16;
    }
    y += 8;
  }
